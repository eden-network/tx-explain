...
async def assets_data(sim_data, network):
    try:
        from_addresses = [x['from'] for x in sim_data['asset_changes']]
        to_addresses = [x['to'] for x in sim_data['asset_changes']]
        ...
        assets = [
            ...
            for from_address, to_address, amount, name, symbol in zip(from_addresses, to_addresses, asset_amounts, asset_names, asset_symbols)
        ]
...
async def correct_summary(sim_data, summary, network):
    try:
...
async def explain_transaction(client, payload, network='ethereum', system_prompt=None, model="claude-3-haiku-20240307", max_tokens=2000, temperature=0, store_result=True):
    ...
    try:
        ...
        corrected_explanation = await correct_summary(payload, explanation, network)
        ...
        for line in lines:
            ...
            for i, word in enumerate(words):
                if i < len(words):
                    yield word + " "
                    await asyncio.sleep(0.01)  # Use asyncio.sleep for asynchronous sleep
                else:
                    yield word
            yield "\n"
